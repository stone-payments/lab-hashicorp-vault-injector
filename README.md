...
* Uma instancia de banco de dados acessível (Aqui estamos usando postgresql no RDS, mas pode ser qulquer Banco de dados suportado pelo Vault). [Aqui](https://bitnami.com/stack/postgresql/helm) você encontra um exemplo para deploy do postgresql dentro do ambiente K8S caso nao queira subir uma instância RDS.
* Uma aplicação que se conecte ao banco de dados acima. Esse repositório contém uma [aplicação](./postgresql-app/) de exemplo para se conectar à um banco postgresql.
